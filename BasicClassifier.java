...
public void printLabelings(Classifier inClassifier, Classifier confusion, String title, InstanceList testInstances, FileWriter[] fos, FileWriter[] fosAlt, int index, String BadLabel) throws IOException {
            Labeling labeling = inClassifier.classify(testInstances.get(index)).getLabeling();
		
	    Labeling ConfLabeling = confusion.classify(testInstances.get(index)).getLabeling();
            // print the labels with their weights in descending order (ie best first)                     

	if(ConfLabeling.getLabelAtRank(0).toString().equals(BadLabel)){
		fosAlt[index].write("Q ");
	 	//System.err.print("Q Produced on " + BadLabel + "\n");
	}
	else{
...
/*	ClearDirectory(ConfName + "SMC/");
	ClearDirectory(ConfName + "SMI/");
	ClearDirectory(ConfName + "STC/");
	ClearDirectory(ConfName + "STI/");
	ClearDirectory(ConfName + "RMC/");
	ClearDirectory(ConfName + "RMI/");
	ClearDirectory(ConfName + "RTC/");
	ClearDirectory(ConfName + "RTI/");
	ClearDirectory(ConfName + "IC/");
	ClearDirectory(ConfName + "II/");*/

	for (int k = 0; k < FOLDS; k ++){	
		System.out.print("Currently on K: " + k + "\n");
		int []K_fold = new int[NUM_ABS];
		...
		String[] empty = new String[0];
        	...
        	InstanceList instancesSMI = this.readDirectory(new File(ConfName + "SM/"),empty);
        	InstanceList instancesSTI = this.readDirectory(new File(ConfName + "ST/"),empty);
        	InstanceList instancesRMI = this.readDirectory(new File(ConfName + "RM/"),empty);
        	InstanceList instancesRTI = this.readDirectory(new File(ConfName + "RT/"),empty);
        	InstanceList instancesII = this.readDirectory(new File(ConfName + "I/"),empty);
		...
		Classifier bayesSMI = this.trainClassifierBayes(instancesSMI);
		Classifier bayesSTI = this.trainClassifierBayes(instancesSTI);
		Classifier bayesRMI = this.trainClassifierBayes(instancesRMI);
		Classifier bayesRTI  = this.trainClassifierBayes(instancesRTI);
		Classifier bayesII = this.trainClassifierBayes(instancesII);
		...
		for(int i = 0; i < testInstances.size(); i ++){
			...
			fosAlt[i] = new FileWriter(new File(AltFileName + foldNum + "/" + AbstractName));
		}

		//Here I print out the stimulus modality stuff...
		for(int index = 0; index < testInstances.size(); index++){
			this.printLabelings(bayesSM, bayesSMI, "StimulusModality", testInstances, fos, fosAlt, index, "SMI");
			this.printLabelings(bayesST, bayesSTI, "StimulusType", testInstances, fos, fosAlt, index, "STI");
			this.printLabelings(bayesRM, bayesRMI, "ResponseModality", testInstances, fos, fosAlt, index, "RMI");
			this.printLabelings(bayesRT, bayesRTI, "ResponseType", testInstances, fos, fosAlt, index, "RTI");
			this.printLabelings(bayesI, bayesII, "Instructions", testInstances, fos, fosAlt, index, "II");
		}
